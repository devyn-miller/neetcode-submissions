...
with temp as (
    select country, year, sum(gold + silver + bronze) as total_medals FROM olympic_medals where category != 'Gymnastics' group by country, year
)

SELECT * FROM temp where total_medals > 20 order by total_medals desc limit 5;
