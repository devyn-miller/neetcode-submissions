...
select country,year,sum(gold+silver+bronze) as total_medals from olympic_medals where category != 'Gymnastics' group by country,year having sum(bronze+silver+gold) > 0
 order by sum(bronze+silver+gold) desc limit 5;
